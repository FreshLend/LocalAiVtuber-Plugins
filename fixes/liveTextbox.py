...
class LiveTextbox():
    ...
    def create_ui(self, lines=10, max_lines=10, label=None):
        textbox = gr.Textbox(lines=lines, container=label is not None, label=label,
                            show_label=label is not None, interactive=True, autoscroll=True)
        gr.Interface(fn=self.message_generator, inputs=[],
                    outputs=[textbox], live=True, flagging_mode='never',
                    submit_btn=gr.Button(visible=False), stop_btn=gr.Button(visible=False), clear_btn=gr.Button(visible=False))
